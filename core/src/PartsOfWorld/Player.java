...
public class Player extends MovingObject implements InputProcessor, IPlayerControls {
	...
	public enum Direction {
		RUN_LEFT, RUN_RIGHT, STAY_LEFT, STAY_RIGHT
	}; // nie dodalem jumpa, bo wtedy nie mozna //by bylo skakac po skosie
	...
	public Player(float x, float y) {
		...
		this.xSpeed = 300;
		this.ySpeed = 0;
		this.gravity = -1500; // takie wartoœci bo jest mno¿one przez delte(¿eby
		// na ka¿dym kompie dzia³alo tak samo)
	...
	// w movement jest zrobiona grawitacja(sciaga playera w dol) i poruszanie w
	// lewo/prawo
	...
	public void movement(float delta) {
		...
		if (y > 0) { // jezeli jest na ekranie
			ySpeed += gravity * delta; // dodajemy grawitacje zeby zwiekszyæ
			// prêdkoœæ spadania
		} else {
		...
		if (dir == Direction.RUN_LEFT)
			moveRight();
		else if (dir == Direction.RUN_RIGHT)
			moveLeft();
	...
	public void draw(SpriteBatch batch) {
		if (canJump) { // jak moze skakaæ to znaczy, ¿e nie jest w powietrzu,
